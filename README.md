...
本版本重点做了同步能力与失败处理体验升级：

- ✅ 实时日志详情弹窗继续加宽，日志行支持更宽显示，长文本更易读
- ✅ 同步库改为基于“已记录仓库地址”全量同步（含 stars 链接扩展出的仓库）
- ✅ 新增 `known_urls.json`，持久化保存已记录的 GitHub 仓库地址与 stars 地址
- ✅ 失败项目分拣改为两类：`404项目` / `失败项目`，并显示每条失败原因
- ✅ 两个失败分类都支持“一键重试”和“一键打开所有 GitHub 页面”，并保留单条打开按钮
...
```bash
cd "/Users/sexyfeifan/Documents/Codex/GitHubCollector"
./scripts/package_macos.sh 1.0.18
```
