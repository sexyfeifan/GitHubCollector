...
本版本重点修复了抓取准确性、README 展示逻辑与统计展示：

- ✅ 首页统计调整为“保存路径 → 目录占用 → 本次流量”，移除累计流量显示
- ✅ 抓取流程升级：先同步仓库源码（git clone / pull），再下载最新 Release 的全部资产
- ✅ README 逻辑重构：简介区“渲染”显示中文精简归纳，“原文”显示 README 原始内容
- ✅ 更新说明中文区保留完整信息，并统一标题为“更新说明”
- ✅ 详情页新增 `README.md` 分组，聚合原始 README + 原始更新记录（Release Notes）
- ✅ 搭建教程提取从 Docker 扩展到通用运行/搭建命令，并按步骤自动标注
...
```bash
cd "/Users/sexyfeifan/Documents/Codex/GitHubCollector"
./scripts/package_macos.sh 1.0.20
```
...
- `v1.0.20`：抓取前同步源码并下载 latest release 全资产、README 展示逻辑重构、统计面板调整
